...
describe("SimC Parser", () => {
  ...
  describe("optimize function", () => {
    it("should optimize double negation", () => {
      ...
      const ifCondition = optimizedAst.conditions?.find(
        (c: any) => c.type === "if",
      );
      expect(ifCondition).toBeDefined();
    ...
    it("should respect optimization options", () => {
      ...
      const ifCondition1 = notOptimizedAst.conditions?.find(
        (c: any) => c.type === "if",
      );
      ...
      const ifCondition2 = optimizedAst.conditions?.find(
        (c: any) => c.type === "if",
      );
      ...
      expect(ifCondition2?.expression.nodeType).toBe(
        ifCondition1?.expression.nodeType === "unary" ? "unary" : "comparison",
      );
    });
  });
